...
namespace CarDealer
{
    public class StartUp
    {
        public static void Main()
        {
            ...
            string input = GetSalesWithAppliedDiscount(context);
        ...
        public static string GetTotalSalesByCustomer(CarDealerContext context)
        {
         ...
         .Where(c => c.Sales.Any()) // Customers who have bought at least 1 car
         .Select(customer => new
         {
             customer.Name,
             BoughtCars = customer.Sales.Count,
             SalesAmounts = customer.Sales.Select(sale => sale.Car.PartsCars.Sum(part => part.Part.Price)),
         ...
         .Select(customer => new
         {
             ...
             SpentMoney = customer.SalesAmounts.Sum(sale => customer.IsYoungDriver ? sale * 0.95m : sale)
         })
         ...
         .ToList();
            ...
            var xmlDocument = new XDocument(
                new XElement("customers",
                    customersTotalSales.Select(customer =>
                        new XElement("customer",
                            new XAttribute("full-name", customer.FullName),
                            new XAttribute("bought-cars", customer.BoughtCars),
                            new XAttribute("spent-money", customer.SpentMoney.ToString("F2", CultureInfo.InvariantCulture))
                        )
                    )
                )
            );
            ...
            // Save the XML document to a file or return it as a string
            string xmlFilePath = "customers-total-sales.xml"; // You can change the file path as needed
            xmlDocument.Save(xmlFilePath);
        ...
        public static string GetSalesWithAppliedDiscount(CarDealerContext context)
        {
         ...
         .Select(sale => new
         {
             Car = new
             {
                 ...
                 TraveledDistance = sale.Car.TraveledDistance
             },
             Discount = sale.Discount,
             CustomerName = sale.Customer.Name,
             Price = sale.Car.PartsCars.Sum(part => part.Part.Price),
             DiscountedPrice = sale.Car.PartsCars.Sum(part => part.Part.Price * (1 - sale.Discount / 100))
         })
         .ToList();

            var xmlDocument = new XDocument(
                new XElement("sales",
                    salesWithDiscounts.Select(sale =>
                        new XElement("sale",
                            new XElement("car",
                                new XAttribute("make", sale.Car.Make),
                                new XAttribute("model", sale.Car.Model),
                                new XAttribute("traveled-distance", sale.Car.TraveledDistance)
                            ),
                            new XElement("discount", sale.Discount.ToString("F2", CultureInfo.InvariantCulture)),
                            new XElement("customer-name", sale.CustomerName),
                            new XElement("price", sale.Price.ToString("F2", CultureInfo.InvariantCulture)),
                            new XElement("price-with-discount", sale.DiscountedPrice.ToString("F2", CultureInfo.InvariantCulture))
                        )
                    )
                )
            );
            ...
            // Save the XML document to a file or return it as a string
            string xmlFilePath = "sales-discounts.xml"; // You can change the file path as needed
            xmlDocument.Save(xmlFilePath);
